...
namespace utils
{
    struct Path
    {
        ...
        Path(const cvrp_joao::Data & data, const BcSolution & bcSolution, double varValue, int id);
    };
    ...
    public:
        AuxiliaryFiles(const BcSolution & bcSolution, const cvrp_joao::Data & data_, const std::string & fileName_,
                       const std::string & type_);
        AuxiliaryFiles(const std::vector<double> & edgesList, const cvrp_joao::Data & data_, const std::string & fileName_,
                       const std::string & type_);
