...
utils::Path::Path(const cvrp_joao::Data & data, const BcSolution & bcSolution, double varValue, int id) :
...
utils::AuxiliaryFiles::AuxiliaryFiles(const BcSolution & bcSolution_, const cvrp_joao::Data & data_,
                                      const std::string & fileName_, const std::string & type_) :
                                      data(data_), type(type_), fileName(fileName_)
{}

utils::AuxiliaryFiles::AuxiliaryFiles(const std::vector<double> & edgesList, const cvrp_joao::Data & data_,
                                      const std::string & fileName_, const std::string & type_) :
                                      data(data_), type(type_), fileName(fileName_)
{}
...
bool utils::AuxiliaryFiles::createSolutionTeXfile(const BcSolution & bcSolution)
{
    ...
    cvrp_joao::Solution solution(bcSolution);

    // get drawing limits
    auto maxX = std::max_element(data.customers.begin(), data.customers.end(),
                                 [](const cvrp_joao::Customer & a,const cvrp_joao::Customer & b) { return a.x < b.x;});
    auto maxY = std::max_element(data.customers.begin(), data.customers.end(),
                                 [](const cvrp_joao::Customer & a,const cvrp_joao::Customer & b) { return a.y < b.y;});
